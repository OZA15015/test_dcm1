...
def vis_data(model, train_loader):
    ...
    x_recon, mean, var, z= model(in_) #VAEモデルにdata投入し, 復元と潜在空間取得
    ...
    for i in range(n):
        ...
        if i == 1:
            plt.title('CVAE_Reconstruction MNIST(10dim)')
        plt.imshow(x_recon[i].reshape(28, 28))
        plt.gray()
        ax.get_xaxis().set_visible(False)
        ax.get_yaxis().set_visible(False)
        plt.show()
    plt.savefig("model0629_10.png")
...
def load_model(train_loader):
    ...
    model.load_state_dict(torch.load("/home/oza/pre-experiment/CVAE/mnist_param/mnist_test1_10.pth", map_location=device))
